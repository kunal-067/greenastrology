 ...
 * sparkle accents, a radiating center starburst, and a faint
 * scattered constellation background.
 *
  ...
  return (
    ...
    >
      <svg
        ...
        height="100%"
        role="img"
        aria-label="Zodiac wheel showing the twelve astrological signs and houses"
      >
        {/* <defs>
          <radialGradient id="zw-bg" cx="50%" cy="42%" r="75%">
            <stop offset="0%" stopColor="#16321f" />
            <stop offset="55%" stopColor="#0a1f12" />
        ...
        <rect x="0" y="0" width="600" height="600" fill="url(#zw-bg)" />

        {/* ---------- faint scattered constellation backdrop ---------- */}
        <g
          className={animated ? "zw-spin" : ""}
        ...
        {[18, 32, 46, 60].map((rr) =>
          Array.from({ length: 16 }, (_, k) => {
            ...
            const p = toXY(rr, a);
            return (
              <circle key={`${rr}-${k}`} cx={p.x} cy={p.y} r={0.9} fill={CREAM} opacity={0.4} />
            );
          })
        )}
      ...
      <style jsx>{`
        ...
        :global(.zw-spin) {
          animation: zw-rotate 240s linear infinite;
        }
        :global(.zw-sparkle) {
