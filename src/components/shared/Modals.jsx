...
import { SS_REVIEWS } from "../sections/TrustBar";
...
export function ReviewModal({ reviewModal, setReviewModal, reviewIdx, setReviewIdx }) {
  return (
    <Modal open={!!reviewModal} onClose={() => setReviewModal(null)}>
      <div className="relative">
        <div className={`aspect-video bg-linear-to-br ${[reviewIdx]?.bg} flex items-center justify-center`}>
          <img className="h-full w-auto" src={SS_REVIEWS[reviewIdx].img} alt={SS_REVIEWS[reviewIdx].label} />
          {/* <p className="text-white/70 font-semibold" style={{ fontFamily: "var(--font-display)" }}>{GALLERY_ITEMS[reviewIdx]?.label}</p> */}
        </div>
        <div className="flex justify-between absolute inset-y-0 left-0 right-0 items-center px-4 pointer-events-none">
          {reviewIdx > 0 && (
            <button onClick={e => { e.stopPropagation(); setReviewIdx(i => i - 1); }} className="w-10 h-10 rounded-full bg-black/50 border border-white/20 flex items-center justify-center text-white hover:bg-black/70 transition-colors pointer-events-auto">
              ‹
            </button>
          )}
          <div />
          {reviewIdx < SS_REVIEWS.length - 1 && (
            <button onClick={e => { e.stopPropagation(); setReviewIdx(i => i + 1); }} className="w-10 h-10 rounded-full bg-black/50 border border-white/20 flex items-center justify-center text-white hover:bg-black/70 transition-colors pointer-events-auto">
              ›
            </button>
          )}
        ...
        <div className="p-4 flex justify-center gap-2">
          {SS_REVIEWS.map((_, i) => (
            <button key={i} onClick={e => { e.stopPropagation(); setReviewIdx(i); }}
              className={`w-2 h-2 rounded-full transition-all ${i === reviewIdx ? "bg-green-400 w-5" : "bg-white/30"}`} />
          ))}
        </div>
      </div>
    </Modal>
    ...
    <Modal open={!!galleryModal} onClose={() => setGalleryModal(null)}>
      <div className="relative">
        <div className={`aspect-video bg-linear-to-br ${GALLERY_ITEMS[galleryIdx]?.bg} flex items-center justify-center`}>
          <img className="h-full w-auto" src={GALLERY_ITEMS[galleryIdx].img} alt={GALLERY_ITEMS[galleryIdx].label} />
        ...
        <div className="p-4 flex justify-center gap-2">
          {GALLERY_ITEMS.map((_, i) => (
            <button key={i} onClick={e => { e.stopPropagation(); setGalleryIdx(i); }}
              className={`w-2 h-2 rounded-full transition-all ${i === galleryIdx ? "bg-green-400 w-5" : "bg-white/30"}`} />
          ))}
        </div>
      </div>
    </Modal>
